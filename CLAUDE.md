...
- **Never set `appearance: none` / `-webkit-appearance: none` on `<input type="color">`** swatches (`.ttrpgmap-color-swatch`, `.ttrpgmap-pin-selector-color`). On the Chromium that ships with recent Obsidian/Electron it breaks the native color control: the swatch stops painting its value (a white marker shows a black swatch) and the picker stops opening / emitting `input` events. Style the swatch via the host border/border-radius plus the `::-webkit-color-swatch` pseudo-elements only. Always pass a canonical `#rrggbb` string to a color input (use `normalizeHexColor()` from `src/utils/colorPicker.ts`) — any other form silently resets the input to `#000000`.
