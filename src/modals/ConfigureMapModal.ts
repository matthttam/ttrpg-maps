...
export class ConfigureMapModal extends Modal {
	...
	onOpen(): void {
		...
		new Setting(mapItems)
			...
			.addText((text) => text.setPlaceholder('500 or 80%').onChange((value) => (this.height = value)));

		new Setting(mapItems)
			.setName('Width')
			.setDesc('Display width (blank = auto from height/image)')
			.addText((text) => text.setPlaceholder('800 or 100%').onChange((value) => (this.width = value)));
		...
		new Setting(zoomItems)
			...
			.addText((text) => text.setValue(String(this.zoomMin)).onChange((v) => (this.zoomMin = parseInt(v, 10) || 50)));

		new Setting(zoomItems)
			.setName('Maximum zoom %')
			.addText((text) => text.setValue(String(this.zoomMax)).onChange((v) => (this.zoomMax = parseInt(v, 10) || 200)));
