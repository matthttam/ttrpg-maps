...
describe('DataManager', () => {
	describe('loadSettings', () => {
		...
		it('ensures predefined default template exists even if data has templates without it', async () => {
			...
			const defaultTemplate = settings.markerTemplates.find((t: any) => t.id === 'default');
	...
	describe('loadMapState', () => {
		...
		it('loads and parses state from file when exists', async () => {
			...
			expect(state.layers).toHaveLength(1);
			expect(app.vault.adapter.read).toHaveBeenCalledWith('.ttrpgmap/map-1.json');
		});
	...
	describe('saveMapState + flushSaves', () => {
		...
		it('flushSaves writes pending saves immediately', async () => {
			...
			expect(app.vault.adapter.write).toHaveBeenCalledWith('.ttrpgmap/map-1.json', JSON.stringify(state, null, 2));
		});
		...
		it('multiple saves for same mapId only writes once after flush', async () => {
			...
			expect(app.vault.adapter.write).toHaveBeenCalledWith('.ttrpgmap/map-1.json', JSON.stringify(state2, null, 2));
		});
	});
	...
	describe('deleteMapState', () => {
		it('removes file when it exists', async () => {
			...
			expect(app.vault.adapter.remove).toHaveBeenCalledWith('.ttrpgmap/map-1.json');
		});
